...
<title>Insert title here</title>
 <script type="text/javascript" src="${pageContext.request.contextPath }/js/jquery.min.js"></script>       
		...
		function dianpu(){
			if("${user}" != null && "${user}" != ""){
				location.href="${pageContext.request.contextPath }/rnfm/selectShop";
			}else{
				location.href="${pageContext.request.contextPath }/jsp/Login.jsp";
			}
		}
...
<div class="bb">寶貝推薦         <c:if test="${!empty user }"><a class="btn ss" href="${pageContext.request.contextPath }/jsp/rnfm.jsp">添加商品</a></c:if></div>	<br/>
 	<div class="row">
 	 	<c:forEach items="${selsctShop}" var="s">
 	 		<div class="col-lg-2 col-md-2 col-sm-2">
 	 		
 	 			<a href="${pageContext.request.contextPath }/shop/GoodId?goodsId=${s.goodsId }">
 				<img src="${s.goodsImg}" width="180px" height="320px"></a><br>
 				<div class="sheng"><a href="${pageContext.request.contextPath }/shop/GoodId?goodsId=${s.goodsId }">${s.goodsInfo }</a></div><br>
 				<span class="se">￥ ${s.shopPrice }</span> &nbsp; &nbsp;<span class="xian">${s.marketPrice }</span>
    			<br>
 				<div>库存 ：${s.goodsStock}</div>
 	 		</div>
 	 	</c:forEach>
 	 </div>
